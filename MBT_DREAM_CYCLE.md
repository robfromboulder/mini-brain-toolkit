...
**Context management:** Phases 1 and 2 have bounded footprints (file listings, cross-doc reads) and are handled directly. Phase 3 is research-heavy, but the `MBT_COMPARABLES.md` and `MBT_BIOLOGY.md` registries bound it: spawn web-search agents (general-purpose with web search; Explore is file-only and won't reach the web) only for the entries §3a/§3b select plus the hunt for new entrants — **not one agent per thread.** Keep the fan-out small and do the synthesis (feature-level cut, positioning, verdicts) in the active context over registry data, not in sub-agents — sub-agent fan-out is the dominant cost of this cycle, and most of it was re-confirming facts the registry already holds. Retrieval/change-check agents don't need high reasoning effort — brief them narrowly and prefer a cheaper/faster model where the harness allows. Phase 4's LOG/FINDINGS read goes to an Explore agent. The active context briefs agents, synthesizes, makes editorial judgments, and makes all edits.
