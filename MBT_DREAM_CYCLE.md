...
5. **Namespace prefix.** `find . working -maxdepth 1 -name '*.md' 2>/dev/null | grep -vE '/(CLAUDE|README)\.md$' | grep -v '/MBT_'` should print nothing — it tolerates a missing or empty `working/` and anchors the lobespace to the start of the filename. Flag any hit.
...
7. **Enforced terms** — if `MBT_ENFORCED_TERMS.md` has entries, grep the canonical docs for each listed term and judge every hit against the entry's enforced sense. A term used in its non-enforced sense is a collision to fix in Phase 2 or flag for the user. An entry whose disambiguation has gone empty (no plausible misuse remains) is a retirement candidate — flag for the user rather than deleting.
...
**Enforced-term candidates.** While reading the LOG and canonical docs, note any word that required disambiguation across sessions — a session that had to stop and ask which sense a word carries, or a collision the check surfaced repeatedly. These are candidates for `MBT_ENFORCED_TERMS.md`; flag them for the user rather than adding entries directly.
