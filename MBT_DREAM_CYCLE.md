...
3. **Version headers.** Every top-level file except the version-exempt ones (`*_LOG.md`, `*_BURNDOWN.md`) carries a well-formed `> V<N>, YYYY-MM-DD.` line — version and date only. Flag any that smuggled a change note into the header.
...
5. **Namespace prefix.** `find . working -maxdepth 1 -name '*.md' 2>/dev/null | grep -vE '/(CLAUDE|README)\.md$' | grep -v '/MBT_'` should print nothing — it tolerates a missing or empty `working/` and anchors the token to the start of the filename. Flag any hit.
