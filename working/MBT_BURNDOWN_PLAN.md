...
- `MBT_PATTERN.md` — three sites: principle 7's exemption phrase ("task checklists"), the stage-3 working-set list `{PLAN,FINDINGS,LOG,TASKS,CLAUDE,TESTING}`, and the stage-3 closeout role row, which gains the early-archival move.
...
- `MBT_CHECK_BRAIN.md`, `MBT_DREAM_CYCLE.md`, `templates/DREAM_CYCLE.md` — each version-header check cites its exemption authority (the pattern for the brain checker, the brain's own `CLAUDE.md` for the dream cycles) rather than carrying the glob.
...
| Version-exemption globs in four documents (the checkers cite the authority instead) | The exemption is written as a filename pattern, not as a doctype name | A renamed file silently becomes version-required, and every brain's checker starts flagging a document that is exempt by design |
...
Then run the check procedure against a brain carrying the new name and confirm the version-header check does not flag the burndown — the exemption site is the one most likely to be missed, because nothing else fails when it is.
...
- Any broader change to how work items are scaffolded, routed, or retired: this renames one document and defines one retirement move for another.
- The plan scaffold's own structure is unchanged.
- A mechanism for verifying that a burndown is complete.
- Migrating existing brains that carry `*_TASKS.md`. The toolkit deliberately carries no legacy-adoption rule — setup knows only BURNDOWN — so the few brains under the old name are corrected by a one-time manual rename per brain, outside any toolkit mechanism.
