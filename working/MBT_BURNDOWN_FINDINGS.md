...
- **A setup rule that adopts legacy TASKS files by renaming them.** Shipped first, then removed. Rejected on reflection: the pattern is too young to accumulate migration debt, and a live TASKS concept — even one setup only knows how to retire — is a standing invitation to refill it with the master list this rename exists to kill. The few brains still under the old name are corrected by a one-time manual rename each, so the toolkit needs no mechanism for it and forgets TASKS entirely.
