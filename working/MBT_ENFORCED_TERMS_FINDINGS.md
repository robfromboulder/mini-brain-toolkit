...
The brain declares a deliberately small set of words whose meaning it **enforces**. Each entry is a prose paragraph that opens with the term in bold and states what the thing *is* in one to three sentences — never its benefits, motivation, history, use cases, or origin — then states what it must not be confused with, so a writer knows which sense to avoid. Each entry's definition invokes no other entry, by name or by relation, so the file reads correctly in any order. Only the disambiguation may name another listed term, and only the one the entry is confused with: when both halves of a confusable pair are listed, neither can be disambiguated without naming the other.
