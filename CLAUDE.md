...
**File naming.** Every mini-brain filename carries the `MBT` lobespace — shared docs as `MBT_*`. Two files are exempt: `CLAUDE.md` (entrypoint) and `README.md` (repo artifact). New top-level knowledge files must include the lobespace; files in `archive/` keep whatever basename they were retired under.
