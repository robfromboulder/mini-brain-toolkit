...
|---|---|---|
| `<LOBESPACE>_<WORK>_PLAN.md` | The spec: objective, what changes, testing approach, scope boundary, open issues | Yes — written from the intake conversation |
...
- **Owning lobe** — in a multi-lobe brain, which lobe the work belongs to: the nearest common ancestor of the lobes the work will concern. That lobe's `working/` holds the docs and its lobespace becomes their `<LOBESPACE>`. Settle this first, since it decides both. A brain with one lobe skips this and uses the brain's own lobespace.
...
**Slug.** Every filename this procedure produces is `<LOBESPACE>_<WORK>_*` — derive `<WORK>` from the work-item name and lead with the owning lobe's lobespace, always. `<WORK>` is a SCREAMING_SNAKE_CASE slug and never carries a lobe's lobespace, its own or another's. A handed SCREAMING_SNAKE identifier is the slug verbatim, minus any lobe's lobespace it opens with, taking the longest match (a handed `<LOBESPACE>_FOO` gives `<WORK>` = `FOO` and files `<LOBESPACE>_FOO_*`); any other handed name gets a short, specific slug written for it. Check the compound `<LOBESPACE>_<WORK>` before use, rewording the slug — handed or written — until both checks pass. Distinctness in the **prefix** sense: the procedures find an item's docs by the glob `<LOBESPACE>_<WORK>_*`, so `<LOBESPACE>_<WORK>_` must not begin any canonical document's name or another open item's compound, and no other item's compound may begin this one's — a slug `PLUM` beside an item slugged `PLUM_JAM` would sweep that item's docs into its own closeout. And in a multi-lobe brain: no other lobe's lobespace may be a longer prefix of `<LOBESPACE>_<WORK>` than `<LOBESPACE>` itself — ownership goes to the longest match, so a parent's slug that continues into a child's lobespace hands the item's files to that child. The fuller name lives in the `# <Work-Item Name>` heading.

**Audit (this is what makes setup safe on partial/existing work items).** Before creating anything, list the owning lobe's `working/` and `archive/` for the slug. From the owning lobe's directory — the repo root for a single-lobe brain:
...
Create each missing doc in the owning lobe's `working/`, from its base template, substituting `<LOBESPACE>` (the owning lobe's lobespace), `<WORK>` (SCREAMING_SNAKE), `<Work-Item Name>` (Title Case), and `<work-branch>` — a single branch name, or a glob when the item will land across several branches/PRs; setup declares the name — creating the branch belongs to the project's workflow, when coding starts. Create that `working/` if the lobe does not have one yet. Fill PLAN and FINDINGS with real content from the intake chat; leave the others as scaffolds for the implementer.
