...
The mirror of this: whoever writes a work item's own closeout notes (in its `<TOKEN>_<WORK>_TASKS.md`) should *not* pre-enumerate this table — which files a work item touches is derived here, at closeout time. A work item's closeout notes record only what this walk won't surface: deviations from the standard flow, and non-derivable callouts — most importantly the specific existing claim a work item **reverses**.
