...
4. **No match** — append to the canonical log of the nearest common ancestor of the lobes the session's work concerned — not every lobe it read: a session that worked inside one lobe logs there, and one whose work crossed lobes logs to the lobe above them, however far apart in the tree they sit. In a brain with one lobe, that is always `<LOBESPACE>_LOG.md`.
