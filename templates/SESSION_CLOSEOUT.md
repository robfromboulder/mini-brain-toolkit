...
4. **No match** — append to the canonical log of the nearest common ancestor of the units the session's work concerned — not every unit it read: a session that worked inside one unit logs there, and one whose work crossed units logs to the unit above them, however far apart in the tree they sit. In a brain with one unit, that is always `<PREFIX>_LOG.md`.
