...
| Purpose | File |
|---|---|
| Problem definition, the world it exists in, goals | `<LOBESPACE>_SCOPE.md` |
| Technical approach, what we build | `<LOBESPACE>_APPROACH.md` |
| Key implementation findings (invisible from code) | `<LOBESPACE>_FINDINGS.md` |
| Session log — why we chose what we chose | `<LOBESPACE>_LOG.md` (read from last `---`; large) |
| Session log update format and rules | `<LOBESPACE>_SESSION_CLOSEOUT.md` (read when asked to update the session log) |
...
**Version header.** Every top-level file except the version-exempt ones opens with `> V<N>, YYYY-MM-DD.` — version and date, nothing else. **No change note in this line.** If a file needs a description, put it on its own line below. Bump `<N>` by one on each substantive edit (numerically — `V10` > `V9`) and set the date. This applies to `CLAUDE.md` itself. Exempt: `README.md`, `CONTRIBUTING.md`, `*_LOG.md` files, and `*_BURNDOWN.md` checklists.
...
|---|---|---|
| `<LOBESPACE>_SCOPE.md` | *none* | The problem statement stands alone; it precedes and outlives any design. |
| `<LOBESPACE>_APPROACH.md` | `<LOBESPACE>_SCOPE.md` only | A design legitimately responds to the problem it addresses — never reaching down into implementation. |
| `<LOBESPACE>_FINDINGS.md` | *none* | Implementation decisions that did *not* shape the approach — downstream of it, so nothing upstream to cite. |
