...
`working/` holds experiments and a work item's in-flight `<PREFIX>_<WORK>_*` docs. When a work item concludes, fold them into the canonical docs and **move** (not delete) them to `archive/`. A doc the item has consumed (commonly the PLAN, once converted to code) may move there early at the developer's request — the work-closeout procedure defines the move.
