...
2. **Disk → read index.** Every top-level `.md` file (excluding `CLAUDE.md`, `README.md`, `CONTRIBUTING.md`, and this file) appears in the read index. Flag orphans.
...
**Model and effort.** Record the model and effort level used for the cycle in the dream log entry header (after the session-ID line, if the entry format includes one). The model name is stated in the system environment context ("You are powered by the model named…"); the effort level may appear in session command output. If either cannot be determined, record it as unknown. This is what makes runs comparable on depth and cost — a low-effort small-model run and a high-effort large-model run are not the same cycle.
