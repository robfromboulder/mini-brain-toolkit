...
5. **Namespace prefix.** Every top-level `.md` except the exempt repo artifacts, and every `working/` doc, carries the `<PREFIX>` token: `ls *.md working/*.md | grep -vE '^(CLAUDE|README)\.md$' | grep -v <PREFIX>` should return nothing. Flag any hit.
