...
- **One term, one meaning.** *Listed terms:* grep the canonical docs, not the logs, for each term the brain's enforced terms file lists; judge every hit against the entry's enforced sense and its disambiguation, and report a misuse as a wording fix. *Unlisted terms:* read the canonical documents as a set and watch for a word carrying two jobs (a domain term in one document borrowed as a generic in another); each reading is defensible alone and the collision shows only side by side. Report each such collision as a candidate enforced term for the user to confirm.
