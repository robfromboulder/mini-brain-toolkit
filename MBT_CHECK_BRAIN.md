...
1. **Read index ↔ disk** — every file in the read index exists; no top-level knowledge file is missing from the index (orphans). *Multi-lobe brain:* run that against the hub index and the hub's documents, then check the registry against disk both ways — every registered lobe has a directory, every lobe directory is registered, at every depth — and check grammar conformance: every lobe carries the full doctype set, and any document beyond it appears in that lobe's **Also holds** cell or its local index.
...
- **Substance findings** — drifted SCOPE claims, a scope that has become a status report, cross-document inconsistencies, a lobe whose scope no longer states one problem, and pruning candidates, if any.
