...
2. **Version headers** — every canonical doc (except those the version-and-date principle in `MBT_PATTERN.md` exempts) opens with a well-formed `> V<N>, YYYY-MM-DD.` and no smuggled change-note.
