...
Rob's session, with Claude as co-author: run a deep code review over this branch, then fix what it found. The review returned nine verified findings — four confirmed defects in the rewritten slug paragraph, three confirmed compliance breaks in the working log, two plausible wording problems — and the fixes landed: the slug paragraph was rewritten with its leading imperative restored, the test plan gained the cases the fixes introduce, and the working log was brought back into spec.

## What the review established

The drafting session's work was committed after its entry closed — the branch's first commit landed the drafted edit together with the working log, and a second commit reworded the slug paragraph twenty-seven minutes later in a session the log never recorded; that rework's diff survives in git but its reasoning is lost, which is itself one of the findings. The rework, while fixing real wording problems, had undone the fix's load-bearing structure: it deleted the leading imperative commanding the token-led filename and moved "not a mechanical transform of the name" — the exact phrase the item's decision record names as root cause — back to the paragraph lead. That left the bare-name path with no commanding instruction at the decision point, and the paragraph contradicting itself about whether the slug is mechanical, when the manual test cases hold only under the verbatim reading. Two further confirmed gaps: the strip rule keyed only on the owning unit's token, so a handed name carrying another unit's token (a hub-token habit in a component brain) would silently embed that token mid-slug; and a mechanical strip could manufacture exactly the parent-continues-into-child compound the same paragraph's longest-match guard forbids, with nothing saying which rule wins.
...
- **The imperative lead is deliberate — pinned in the toolkit findings** — Rob's call, made after walking the paragraph's git history, which showed its form flipping on three consecutive edits: imperative-led, then demoted back to definition-led by the unlogged rework, then restored by this session's fix. Rob doesn't recall directing that demotion and judges it a mistake, though it's unknowable now — sessions have run on different models to manage cost, and only a log entry could have said which this was. The toolkit findings now record the settled form so a future wording pass can't re-fight it silently.

## Lessons

A fix whose substance is structure — an imperative at a decision point — can be reworded to death while every individual sentence still looks right; the regression was caught only by re-checking the final text against the item's own decision record and test cases. And an unlogged rework session costs double: its rationale is unrecoverable, and every later session pays to reconstruct what it did from the diff. When sessions run on different models, the log is also the only thing that separates a decision from a cheaper model's drift — unlogged, the two are indistinguishable in the diff.

---

# SETUP_PREFIX work item closed out and folded into the canonical docs (2026-08-14)

**Session ID**: `a38415b7-411f-4909-82dd-99958be6c31a`

Rob's session, with Claude (Opus 4.8) as co-author. With the prefix-enforce branch merged to main earlier this session (PR #3), Rob declared the SETUP_PREFIX work item done and directed a work-item closeout, held short of any commit so the result can be reviewed first. Its two durable, file-invisible findings were folded into the toolkit findings, its two working-log sessions merged whole into this log, and its five working docs moved to `archive/`.

## Turn-by-turn

- The target walk found no SCOPE or APPROACH claim touched — the change is a sharpening of template prose, not a factual or design-assumption change — and the toolkit carries no canonical testing or runbook doc, so the item's own test plan and runbook retire to `archive/` without folding. The two live targets were the findings and this log.
- Two findings were promoted to the toolkit findings, both invisible from the shipped template: that a model-executed instruction fails when its load-bearing rule sits in definitional prose and is fixed by an imperative at the decision point — carrying the pin that this paragraph's imperative-led form regressed once in an unlogged session and must not be re-demoted — and that the redundant work-item token was kept rather than dropped because removing it is unconfined, its pattern-wide retirement deferred to a separate redesign. The decision record's problem statement and Option-B mechanics were omitted as now spelled out in the shipped `WORK_SETUP` paragraph itself.
- The item's two working-log sessions — the 2026-08-13 draft and the 2026-08-14 review-and-repair — were merged whole into this log in date order, each keeping its original session ID; their references to the item's own sibling docs were rewritten to point at the toolkit findings where that knowledge now lives, or to the decision by its substance.
- No reversal: `WORK_SETUP` already intended the prefix, so nothing canonical became false and the reconcile step had nothing to supersede.
- The runbook, decision record, plan, test plan, and working log were moved to `archive/` under their working basenames.
...
The findings outside the collapse were fixed alongside it. The sub-lobe registry row regained a distinct placeholder, and the vocabulary check's grep list and replacement mapping gained the old sub-lobe placeholder too, so a brain migrating from the old vocabulary carries the distinction across rather than flattening it on the way. The pattern definition now says each *child* lobe takes its own subdirectory, which is what the create procedure had said all along. And the approach doc's surviving pattern-sense "components" was renamed — notable because the branch had already version-bumped that file, so the miss sat inside the edited surface rather than in an untouched one.
...
- **Two ambiguous uses resolved by a fresh-reader test, not by the namespace question** (Rob's prompt, Claude's fix). Claude first proposed leaving two remaining uses of the old term in the implementation-findings doc, reasoning that neither carries the namespace sense the rename covers. Rob asked instead how they read to someone who had never seen the old definition — the better test, and it reversed the answer. One sits in a parenthetical beside "files" and "dir" while naming a position in a filename grammar, and the multi-lobe layer already teaches "slot" for exactly that, so the reader meets two words for one concept: the defect the rename exists to remove. The other contrasts a literal string with an idea and is decodable on its own, but it is a third sense with nothing in the repo to arbitrate, and it invites the slot reading for a doctype that was in fact struck as a name. Renamed to "slot" and "name"; both editorial, so no version bump. The word now survives only in its ordinary-English senses, and the vocabulary check goes quiet on the file instead of re-surfacing it on every run.
...
# Dry-run validation of CHECK_BRAIN across three existing brains (2026-09-22)

**Session ID**: `a74593cc-cbd0-4731-ac51-b6065b7aca72`

Rob's session, with Claude (Opus 4.6) as co-author. Before merging PR #5, Rob asked for dry-run validation of `MBT_CHECK_BRAIN.md`'s vocabulary-alignment walkthrough against three existing brains of different shapes. Each dry run created a temporary worktree, seeded it with the target brain's current files, ran the check procedure's vocabulary scan, classified every hit, applied the conversion, and verified the final state. The session found three gaps in CHECK_BRAIN and fixed them on the branch before merge.

## The three brains

- **mini-nebula-brain** (single-lobe, mature) — exercised the `<PREFIX>` compound convention, where the brain had redefined `<PREFIX>` as `NEBULA_<WORK>` rather than just the bare lobespace. The walkthrough's `<PREFIX>` → `<LOBESPACE>` mapping would have dropped the `<WORK>` slot and changed semantics. The correct conversion expanded the compound to its concrete form.
- **mini-okx-brain** (single-lobe, new/immature) — minimal vocabulary hits, all template-carried. Confirmed that templates on the branch are already converted, so brains seeded after merge would get clean vocabulary. Also surfaced the commented-out read-index false positive.
- **virtual-view-brain** (multi-lobe: hub VV + manifesto/VVM, viewmapper/VMR, viewzoo/VZOO) — the broadest test case, exercising every mapping simultaneously: `<TOKEN>`, "token", "component", "unit" across hub docs, procedure docs, and sub-lobe working files. Context-sensitive classification correctly distinguished pattern-specific uses from ordinary English ("metadata component", "token limits", "unit of dependency", "unit vs. integration").

## Findings and fixes

Three gaps in CHECK_BRAIN were identified and committed to the branch:

1. **`<PREFIX>` compound convention** (check 7 walkthrough) — the walkthrough prescribed `<PREFIX>` → `<LOBESPACE>`, but brains that defined `<PREFIX>` as a compound (`LOBESPACE_<WORK>`) need the compound expanded to its concrete form rather than a simple rename. Added a note to the walkthrough.
2. **Commented-out read-index entries** (check 1) — the read-index ↔ disk check picked up HTML-commented entries (`<!-- ... -->`) as false positives. Added guidance to ignore them.
3. **README.md omitted from vocabulary scan** (check 7) — the scan targeted "canonical docs and `CLAUDE.md`" but README.md also carries pattern vocabulary. Added README.md to the scan targets.

## Decisions

- **Collect all findings before changing CHECK_BRAIN** (Rob's direction) — all three dry runs completed before any fixes were applied, so the fixes could address the full finding set at once.
- **Exclude mini-guardrails-brain** (Rob's decision) — the oldest brain, not yet converted to multi-lobe format; not a useful test for the vocabulary walkthrough.

PR #5 merged after the fixes landed.
